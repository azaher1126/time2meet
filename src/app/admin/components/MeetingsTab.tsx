...
export function MeetingsTab({ meetings }: MeetingsTabProps) {
  ...
  }, [meetings, searchTerm, dateRangeStart, dateRangeEnd, minResponses, maxResponses, ownerSearch, showAnonymousOnly, visibilityFilter]);
  ...
  const columns: Column<AdminMeeting>[] = [
    {
      ...
      render: (meeting) => (
        <div>
          ...
          <div className="text-sm text-gray-500 font-mono">/m/{meeting.share_link}</div>
        </div>
      ),
    },
    {
      ...
      header: "Creator",
      sortable: true,
      sortValue: (meeting) => meeting.creator_name || meeting.creator_email || "",
      render: (meeting) =>
    ...
    },
    {
      ...
      render: (meeting) => (
        <span className="text-sm text-gray-500">
          {format(parseISO(meeting.start_date), "MMM d")} - {format(parseISO(meeting.end_date), "MMM d, yyyy")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (meeting) => (
        <span className="text-sm text-gray-500">{format(parseISO(meeting.created_at), "MMM d, yyyy")}</span>
      ),
    },
    {
      key: "actions",
        ...
        {showFilters && (
          <div className="pt-4 border-t border-gray-200 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {/* Owner Filter */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Owner (Name or Email)</label>
                <input
                  type="text"
                  value={ownerSearch}
                  onChange={(e) => {
